...
%Bifurcation parameter E, quasistatically varying from 1 to 7
Espan=linspace(1,7,10);
...
%seed value
rng(100)
%Random initial conditions in synchronous subspace
cells0=repmat(rand(2,1),[p^2,1]);
%cells0=repmat([1.409;1.544],[p^2,1]); %near synchronous state
%cells0=rand(2*p^2,1)*10^(-6);
%cells0=ones(2*p^2,1);
...
%Run ODE while quasistatically changing E and W
for E=Espan
    W=E; %linear path through parameter space (plane with axes E and W)
        ...
        %Set yfinal+noise to be initial conditions for next run
        cells0=yfinal'+rand(2*p^2,1)*10^(-6);
end

%Now yfinal is the run for the last E value in Espan, and we do the
%analysis on this yfinal
%Separate vector into MapK and Notch components
